...
def constantes_const_int():
    ...
    tk.Label(constantes_const_int_window, text="Nom de la constante const int (ex: ledPin):").pack()
...
def constantes_int():
    ...
    tk.Label(constantes_int_window, text="Nom de la variable int (ex: compteur):").pack()
...
def constantes_float():
    global constantes_float_window, nom_constante_entry_float, valeur_constante_entry_float
    constantes_float_window = tk.Toplevel(root)
    constantes_float_window.title("Variables Entières (float)")

    tk.Label(constantes_float_window, text="Nom de la variable float (ex: compteur):").pack()
    nom_constante_entry_float = tk.Entry(constantes_float_window)
    nom_constante_entry_float.pack()

    tk.Label(constantes_float_window, text="Valeur de la variable (ex: 0):").pack()
    valeur_constante_entry_float = tk.Entry(constantes_float_window)
    valeur_constante_entry_float.pack()

    def valider_constante_float():
        nom = nom_constante_entry_float.get()
        valeur = valeur_constante_entry_float.get()
        try:
            try:
                float(valeur)
            except ValueError:
            ...
            try:
                with open("fichier.txt", "r+") as fichier:
                    contenu = fichier.read()
                    nouveau_contenu = re.sub(r"(void setup\(\))", f"const float {nom} = {valeur};\n\\1", contenu, flags=re.MULTILINE)
                    fichier.seek(0)
                    fichier.write(nouveau_contenu)
                    fichier.truncate()
            except FileNotFoundError:
            ...
            nom_constante_entry_int.delete(0, tk.END)
            valeur_constante_entry_int.delete(0, tk.END)
            messagebox.showinfo("Succès", "Constante ajoutée.")

        except Exception as e:
            messagebox.showerror("Erreur", f"Une erreur s'est produite : {e}")

    def suivant_constante_float():
        fin()
        constantes_float_window.destroy()

    tk.Button(constantes_float_window, text="Valider", command=valider_constante_float).pack()
    tk.Button(constantes_float_window, text="Suivant", command=suivant_constante_float).pack()

def fin():
    global fin_window
    fin_window = tk.Toplevel(root)
    fin_window.title("fin")

    tk.Label(fin_window, text="voila la 1.0 je travaille sur la suite sur mon temps libre donc les versions suivant arriverons plus tard").pack()
    tk.Label(fin_window, text="si vous avez d'autre question le programme est open source est si vous avez des questions sur arduino").pack()
    tk.Label(fin_window, text="https://docs.arduino.cc").pack()
    
    def terminer():
        fin_window.destroy()

    tk.Button(fin_window, text="fin", command=terminer).pack()
